...
/// The PlayerHealth script controls the health of the player and allows for TakeDamage function and a block ablity.
public class PlayerHealth : MonoBehaviour
{
    ...
    /** If the player is currently invulnerable, this method does not not do anything.
        ...
        If the player is not blocking a decrease in health will happen, the amount depending on the provided parameter. They also gain invulnerablility for a set amount of time.
        If the player has zero or less health they are sent back to the hub. */
    public void TakeDamage(float x, float _knockback = 10f, float _angle = 45f)
    {
        ...
        else
        {
            ...
            StartCoroutine(SetInvulnerableForSeconds(_invincibilitySeconds));
        }
    ...
    /// Sets the player invulnerable for a specified amount of seconds.
    IEnumerator SetInvulnerableForSeconds(float _time)
    {
